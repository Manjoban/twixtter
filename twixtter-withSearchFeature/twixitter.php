...
{   
	...
	$menu = "<a href='twixitter.php?action=addpost'>Make a New Post</a> | <a href='twixitter.php?action=account'>My Account</a> | <a href='twixitter.php?action=logout'>Logout</a>|
	<a href='twixitter.php?action=search'>Search</a>";
}
else
...
elseif ($action == "Search Results") {
    ...
    if (!empty($query)) {
        ...
        $searchResults = searchUsersAndPosts($query);
        include("view/header.php");
        include("view/search_results.php");
    } else {
        $error = "<p>Please enter something to search.</p>";
        $pagetitle = "Search";
        include("view/header.php");
        include("view/search_form.php");
    }
}
elseif ($action == "search" && $loggedin) {
    $pagetitle = "Search";
    include("view/header.php");
    include("view/search_form.php");
} 
elseif ($action == "Search Posts" && $loggedin) {
    $search = trim(filter_input(INPUT_POST, 'query'));
    if (!empty($search)) {
        saveSearchHistory($account_id, $search, 'posts');
        $results = searchPosts($search);
        $pagetitle = "Search Post Results";
        include("view/header.php");
        include("view/search_results.php");
    } else {
        $error = "<p>Please enter a post term to search.</p>";
        include("view/header.php");
        include("view/search_form.php");
    }
}
elseif ($action == "Search Users" && $loggedin) {
    $search = trim(filter_input(INPUT_POST, 'query'));
    if (!empty($search)) {
        saveSearchHistory($account_id, $search, 'users');
        $results = searchUsers($search);
        $pagetitle = "Search User Results";
        include("view/header.php");
        include("view/search_results.php");
    } else {
        $error = "<p>Please enter a username to search.</p>";
        include("view/header.php");
        include("view/search_form.php");
    }
}
elseif ($action == "Search Results" && $loggedin) {
    $search = trim(filter_input(INPUT_GET, 'query'));
...
elseif ($action == "Search History" && $loggedin) {
    $history = getSearchHistory($account_id);
    $pagetitle = "My Search History";
    include("view/header.php");
    include("view/search_history.php");
}
