...
function createPostsList($posts, $account_id)
{
	if ($posts != NULL) {
		...
		foreach ($posts as $row) {
			...
			$liked = hasLiked($account_id, $id);

			$postsList .= "<p>$message</p><p><b>Posted by</b> $username on $posted_date</p>";
			$postsList .= "<p><b>$likeCount</b> Likes</p>";

			// Like/Unlike link
			if ($account_id > 0) {
				if ($liked) {
					$postsList .= "<p><a href='twixitter.php?action=unlike&id=$id'>Unlike</a></p>";
				} else {
					$postsList .= "<p><a href='twixitter.php?action=like&id=$id'>Like</a></p>";
				}
			}

			// Show Edit/Delete links if user owns the post
		...
		$postsList = "<h2>No posts found.</h2>";
	}
	return $postsList;
}
...
{
	global $db;
	$query = 'INSERT IGNORE INTO likes (account_id, post_id) VALUES (:account_id, :post_id)';
	$statement = $db->prepare($query);
	$statement->bindValue(':account_id', $account_id);
...
function unlikePost($account_id, $post_id)
{
	global $db;
	$query = 'DELETE FROM likes WHERE account_id = :account_id AND post_id = :post_id';
	$statement = $db->prepare($query);
	$statement->bindValue(':account_id', $account_id);
	$statement->bindValue(':post_id', $post_id);
	$statement->execute();
...
function hasLiked($account_id, $post_id)
{
	global $db;
	$query = 'SELECT * FROM likes WHERE account_id = :account_id AND post_id = :post_id';
	$statement = $db->prepare($query);
	$statement->bindValue(':account_id', $account_id);
	$statement->bindValue(':post_id', $post_id);
	$statement->execute();
...
function getLikedPosts($account_id)
{
	global $db;
	$query = 'SELECT p.*, a.username FROM posts p
